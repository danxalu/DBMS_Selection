...
**Почему:**  
Топ-производительность для агрегирующей аналитики, дешёвая длительная история за счёт компрессии, гибкие партиции (пример: `toYYYYMM(date)` × регион), материализованные витрины, нативный стрим-ингест (Kafka).  

**Как внедрять:**  
1. CDC из OLTP (PostgreSQL) -> Kafka -> ClickHouse (`Kafka Engine` + `Materialized View`).  
2. Модель «звезда»: факты продаж + измерения (пример: клиент, продукт, филиал, регион, календарь).  
...
- Модель «звезда», партиционирование по месяцу/году, кластеризация (пример: по региону/продукту).  
...
- Включить **parallel query** и **JIT**; тюнинг `work_mem`, `effective_cache_size`.  
